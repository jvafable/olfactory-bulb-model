...
```{r}
setwd("C:/Users/JV/Desktop/masters/data0_default")
...
title(xlab = "MC #", ylab = "Input (nA)", cex.lab = 1.2, font.lab = 2)

barplot(x_values, col="black",
        ylab="", yaxt="n", ylim=c(0, 40))
axis(1, at=0)
axis(2, at=seq(0, 40, 20))
title(xlab = "MC #", ylab = "Firing rate (Hz)", cex.lab = 1.2, font.lab = 2)
```
...
plot_pair <- function(cell1, cell2, type) {
    ...
    lwd <- 2
    plot(tt, vs1, type="l", 
         xlim=c(2000, 3000), xlab="",
         ylim=c(-80, 40), ylab="mV",
         col="blue", lwd=lwd)
...
compare_pairs <- function(pair1, pair2, type) {
    par(mfrow=c(2, 1), mai=c(0.4,0.8,0.5,0.4))
    plot_pair(pair1[[1]], pair1[[2]], type)
    title(type)
    par(mai=c(0.9, 0.8, 0.1, 0.4))
...
plot_spike_diff <- function(type, i, j) {
    ...
    hist(spike_diff, breaks=seq(-3000, 3000, 5), xlab="",  ylab="", main="")
...
```{r}
plot_solo <- function(cell1, type) {
    prefix <- c("MC" = "Vms", "GC" = "Vgb", "PGC" = "Vpb")[type]
    vs1_name <- sprintf("%s_%d_%d", prefix, cell1[1], cell1[2])
    vs1 <- read.table(vs1_name)$V1
    # Assuming tt has been loaded
    lwd <- 2
    plot(tt-2000, vs1, type="l", 
         xlim=c(0, 1000), xlab="ms",
         ylim=c(-80, 40), ylab="mV",
         col="blue", lwd=lwd)
} 

plot_solo(c(2, 1), type="MC")
```
