...
#define gmm gmm_kamt
 double gmm = 0.75;
#define htau htau_kamt
 double htau = 0;
#define hinf hinf_kamt
 double hinf = 0;
#define mtau mtau_kamt
 double mtau = 0;
#define minf minf_kamt
 double minf = 0;
#define q10 q10_kamt
 double q10 = 3;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kamt /home/jv/OBGAMMA/kAmt.mod\n");
...
static int  trates (  double _lv ) {
 ...
 _lqt = pow( q10 , ( ( celsius - 24.0 ) / 10.0 ) ) ;
   ...
   htau = k_tauH * beth ( _threadargscomma_ _lv ) / ( _lqt * a0h * ( 1.0 + alph ( _threadargscomma_ _lv ) ) ) ;
    return 0; }
...
double alph (  double _lv ) {
   double _lalph;
 _lalph = exp ( zetah * ( _lv - vhalfh - sh_tauH ) ) ;
...
double beth (  double _lv ) {
   double _lbeth;
 _lbeth = exp ( zetah * gmh * ( _lv - vhalfh - sh_tauH ) ) ;
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 55 in file kAmt.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/jv/OBGAMMA/kAmt.mod";
static const char* nmodl_file_text = 
  "TITLE K-A\n"
