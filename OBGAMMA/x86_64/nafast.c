...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nafast /home/jv/OBGAMMA/nafast.mod\n");
...
double alp (  double _lv , double _li ) {
   double _lalp;
 if ( _li  == 0.0 ) {
     _lalp = 0.32 * expM1 ( _threadargscomma_ - ( _lv * 1.0 + 42.0 - sh ) , 4.0 ) ;
     }
   else if ( _li  == 1.0 ) {
     _lalp = 0.128 / ( exp ( ( _lv * 1.0 + 38.0 - sh ) / 18.0 ) ) ;
     }
...
double bet (  double _lv , double _li ) {
   double _lbet;
 if ( _li  == 0.0 ) {
     _lbet = 0.28 * expM1 ( _threadargscomma_ _lv * 1.0 + 15.0 - sh , 5.0 ) ;
     }
   else if ( _li  == 1.0 ) {
     _lbet = 4.0 / ( exp ( - ( _lv * 1.0 + 15.0 - sh ) / 5.0 ) + 1.0 ) ;
     }
 ...
 v=_v;
{
  ena = _ion_ena;
 { error =  states();
 if(error){fprintf(stderr,"at line 45 in file nafast.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/jv/OBGAMMA/nafast.mod";
static const char* nmodl_file_text = 
